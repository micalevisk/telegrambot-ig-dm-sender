// This file fixes `getInbox` method from https://github.com/linkfy/Tools-for-Instagram/blob/083abc8a524024c7ee26da3ef017260838f53d92/src/getInbox.js
...
async function getInbox(ig, extraInfo = new Object()){
    ...
    let directMessagesFormated = threads.map(function(dm) {
        ...
        let readByUser = (dm.thread_title == '') || false; //By default the user did not read the message if there is no timestamp unless the message was sent to himself
        let readByMe = true; //By default you readed the message if there is no answer timestamp by user
        const userId = Object.keys(dm.last_seen_at).find(pk => pk != myId); // This value is undefined when the DM is with yourself or the other is is blocked you user or for another reason

        if(dm.last_seen_at[myId] != undefined) {
          readByMe = (dm.last_seen_at[myId].timestamp >= dm.last_permanent_item.timestamp);
        }
        if(dm.last_seen_at[userId] != undefined) {
